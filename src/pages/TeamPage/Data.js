export const equipe = {
  lightBg: true,
  primary: false,
  titre: "Notre équipe",
  description:
    "Toute notre équipe est formée à la thérapie DBT auprès de l’institut de formation BTECH. Nous proposons des protocoles DBT, ainsi que d’autres approches thérapeutiques qui viennent compléter le traitement.  Le centre se compose de 4 psychiatres, 4 psychologues, et un thérapeute pratiquant l'EMDR, l'hypnose et l'acupression.  ​L'équipe proposant les protocoles DBT est constituée de 2 psychiatres et 4 psychologues : Pour la prise de rendez-vous DBT, il faut d'abord prendre rendez-vous avec un médecin psychiatre. Pour prendre rendez-vous cliquez sur le nom du praticien (redirection Doctolib) ou adressez un mail à l’adresse indiquée.",
  membres: [
    {
      nom: "Dr. Luís Alvarez",
      specialite: "Psychiatre et pédopsychiatre",
      TeamMemberMail: "team.psychologues@MemberMail.com",
      doctolibLink: "https://www.doctolib.fr/psychiatre-de-l-enfant-et-de-l-adolescent/paris/luis-alvarez-zalamea",
      image: require("../../images/luis_profile.jpeg"),
    },
    {
      nom: "Dr. Laure Woestelandt",
      specialite: "Psychiatre et pédopsychiatre",
      TeamMemberMail: "team.psychologues@MemberMail.com",
      doctolibLink: "https://www.doctolib.fr/psychiatre/paris/laure-woestelandt-paris",
      image: false,
    },
    {
      nom: "Philippine Lebert",
      specialite: "Psychologue clinicienne d'orientation intégrative",
      TeamMemberDescription:
        "Psychologue clinicienne d'orientation intégrative, formée au programme 'DBT S' par le Psychwire. Diplômée de l'École de Psychologues Praticiens, elle considère la psychothérapie comme un moyen d'accompagner chaque personne vers un mieux-être, dans une relation de collaboration et d'échange, en fonction de ses objectifs propres. Son approche intègre différentes techniques et méthodes pour répondre aux besoins uniques de chaque individu qu'elle rencontre.",
      TeamMemberMail: "team.psychologues@MemberMail.com",
      doctolibLink: "",
      image: "",
    },
    {
      nom: "Laura Lunel",
      specialite: "Psychologue clinicienne et thérapeute DBT",
      TeamMemberDescription:
        "Psychologue clinicienne et thérapeute DBT. Formée en DBT intensive dispensée par Behavioral Tech, elle s'est spécialisée dans les prises en charge du psychotraumatisme, notamment avec une formation avec Psychwire.",
      TeamMemberMail: "team.psychologues@MemberMail.com",
      doctolibLink: "",
      image: require("../../images/laura_profile.jpeg"),
    },
    {
      nom: "Maya Dewsbery",
      specialite: "Psychologue clinicienne",
      TeamMemberDescription:
        "Psychologue clinicienne exerçant dans le 12ème arrondissement.",
        TeamMemberMail: "team.psychologues@MemberMail.com",

      doctolibLink: "https://www.doctolib.fr/psychologue/paris/maya-dewsbery",
      image: require("../../images/maya_profile_picture.jpeg"),
    },
    {
      nom: "Paula Pellegrini",
      specialite: "Psychologue clinicienne",
      TeamMemberMail: "team.psychologues@MemberMail.com",
      doctolibLink: "",
      image: "",
    },
    {
      nom: "Inès Clowez",
      specialite: "Psychologue clinicienne",
      TeamMemberDescription:
        "Psychologue clinicienne, diplômée de l’École de Psychologues Praticiens, formée au programme 'DBT Skills' ainsi qu’au programme 'DBT Intensive course' par l’institut de formation BTECH.",
      TeamMemberMail: "team.psychologues@MemberMail.com",
      doctolibLink: "",
      image: "",
    },
  ],
};


export const partenaires = {
  titre: "Nos partenaires",
  description:
    "Nous travaillons en collaboration avec les partenaires suivants :",
  liste: [
    {
      nom: "Anthony LAROCHE",
      specialite:
        "Hypnothérapeute, Maitre-Praticien certifié en hypnose ericksonienne",
      description:
        "Il travaille en thérapie brève depuis plus de 10 ans et est spécialisé dans la libération émotionnelle liée, notamment, à tout type d’événement traumatique.",

    },
    {
      nom: "La Clinique des Femmes",
      description:
        "Nous prenons soin des femmes. De toutes les femmes et sans jugement. De la petite enfance à la ménopause. De toute la santé : gynécologique et obstétricale mais aussi psychique et émotionnelle.",
      contact: "01 44 25 00 00",
      adresse: "95 avenue de Bourdonnais, 75007 Paris",

    },
    {
      nom: "American Hospital of Paris",
      specialite: "Diagnostic prénatal",
      description:
        "Un accès à tous les experts du dépistage et du diagnostic anténatal en un seul lieu pour réaliser l’ensemble des examens et consultations : échographies, one day test, dépistage prénatal non invasif et invasifs, génétique.",
      contact: "01 46 41 28 82",
      eTeamMemberMail: "centre-diagnostic-prenatal@ahparis.org",
      adresse: "55 boulevard du Château, 92200 Neuilly-sur-Seine",

    },
  ],
};
